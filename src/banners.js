 ...
 */
  ...
  */
   ...
   * @param {DataCore} 
   */
  this.DC = new DataCore(rRoot, 'banners');
  ...
  */
  this.getDataFromApi = function(granularity, params) {
    ...
    if (response.reportId != undefined && response.totalCount != undefined) {
      ...
      this.Root.Log.addRecord('Nepodařilo se získat reportId nebo totalCount z createReportu', true, 'BannersClass.getDataFromApi()');
      this.Root.Log.addValue(response, true, 'BannersClass.getDataFromApi()');
      return false;
    }
  }
  ...
  this.readReportRoot = function(reportId, totalCount, limit) {
    ...
    if (totalCount > limit) {
      this.Root.Log.addRecord('Počet bannerů je více než povolená hodnota - požadujete '+totalCount+' a limit je nastaven na '+limit+'.'
    ...
    for (var c = 0; c < cycle && c < 3; c++) {
      ...
      if(!response) {
        this.Root.Log.addDebug('-//- Jedna z odpovědí je FALSE', 'Banners.readReportRoot()');
        return false;
      }
      if (c == 0) {
    ...
    this.Root.Log.addDebug('-//- Complete reponse from', 'Banners.readReportRoot()', completeResponse); 
  ...
  this.readReport = function(reportId, offset, limit) {
    return this.Root.sklikApiCall(
      ...
      reportId,
      {
        ...
        'displayColumns': this.DC.getColumns('banners')
      }],
      1
    );
  }
  ...
  this.convertDataToGDS = function (response) {
    return this.DC.returnDataPackage(response, 'banners');
  }
  ...
  this.convertDataToGDSInGranularity = function (response) {
    return this.DC.returnDataPackageInGranularity(response, 'banners');
  }
}