 ...
 */
  ...
  */
   ...
   * @param {DataCore} 
   */
  this.DC = new DataCore(rRoot, 'ads');
  ...
  */
  this.getDataFromApi = function(granularity, params) {
    ...
    if (response.reportId != undefined && response.totalCount != undefined) {
      ...
      this.Root.Log.addRecord('Nepodařilo se získat reportId nebo totalCount z createReportu', true, 'AdsClass.getDataFromApi()');
      this.Root.Log.addValue(response, true, 'AdsClass.getDataFromApi()');
      return false;
    }
  }
  ...
  this.readReportRoot = function(reportId, totalCount, limit) {
    ...
    if (totalCount > limit) {
      this.Root.Log.addRecord('Počet reklam je více než povolená hodnota - požadujete '+totalCount+' a limit je nastaven na '+limit+'.'
    ...
    for (var c = 0; c < cycle && c < 3; c++) {
      ...
      if(!response) {
        this.Root.Log.addDebug('-//- Jedna z odpovědí je FALSE', 'Ads.readReportRoot()');
        return false;
      }
      if (c == 0) {
    ...
    this.Root.Log.addDebug('-//- Complete reponse from', 'Ads.readReportRoot()', completeResponse); 
  ...
  this.readReport = function(reportId, offset, limit) {
    return this.Root.sklikApiCall(
      ...
      reportId,
      {
        ...
        'displayColumns': this.DC.getColumns('ads')
      }],
      1
    );
  }
  ...
  this.convertDataToGDS = function (response) {
    return this.DC.returnDataPackage(response, 'ads');
  }
  ...
  this.convertDataToGDSInGranularity = function (response) {
    return this.DC.returnDataPackageInGranularity(response, 'ads');
  }
}