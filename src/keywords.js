 ...
 * @param {Root} rRoot 
 */
var KeywordsClass = function (rRoot, campaignsId) {
  /**
  * @param {Root}
  */
  this.Root = rRoot;
   ...
   * @param {DataCore} 
   */
  this.DC = new DataCore(rRoot, 'keywords');

  /**
   * @param {Array|False}
   * Keywords will be only from this campaignsId (or from all account on False)
   */
  ...
  */
  this.getDataFromApi = function(granularity, params) {
    ...
    if (response.reportId != undefined && response.totalCount != undefined) {
      ...
      this.Root.Log.addRecord('Nepodařilo se získat reportId nebo totalCount z createReportu', true, 'KeywordClass.getDataFromApi()');
      this.Root.Log.addValue(response, true, 'KeywordClass.getDataFromApi()');
      return false;
    }
  }
  ...
  this.readReportRoot = function(reportId, totalCount, limit) {
    ...
    if (totalCount > limit) {
      this.Root.Log.addRecord('Počet klíčových slov je více než povolená hodnota - požadujete '+totalCount+' a limit je nastaven na '+limit+'.'
  ...
  this.readReport = function(reportId, offset, limit) {
    return this.Root.sklikApiCall(
      ...
      reportId,
      {
        ...
        'displayColumns': this.DC.getColumns('keywords')
      }],
      1
    );
  }
  ...
  this.convertDataToGDS = function (response) {
    return this.DC.returnDataPackage(response, 'keywords');
  }
  ...
  this.convertDataToGDSInGranularity = function (response) {
    return this.DC.returnDataPackageInGranularity(response, 'keywords');
  }
}